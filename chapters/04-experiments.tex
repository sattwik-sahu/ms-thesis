...
Having established the theoretical formulation of the task-conditioned Energy-Based Model, this chapter details the empirical framework utilized to evaluate its predictive and representational capabilities. The experimental design is structured to systematically assess three core competencies of the proposed architecture: the stability of the joint optimization process, the fidelity of the language-grounded autoregressive rollouts, and the geometric integrity of the learned latent manifold. By bridging abstract latent representations with observable metrics, we aim to validate the model's efficacy as a reliable ``imagination engine'' for downstream control.

The remainder of this chapter is organized as follows. \cref{sec:training_setup} outlines the training configuration, critical architectural modifications (such as the task gain factor), and the custom robotic manipulation dataset utilized for learning. \cref{sec:generative_viz} introduces the convolutional decoding mechanism used to project latent trajectories back into human-interpretable visual sequences. \cref{sec:task_performance} defines the quantitative metrics, notably the Task Sensitivity Index ($\beta$), developed to measure the model's discriminative conditioning on counterfactual linguistic instructions. Finally, \cref{sec:manifold_analysis} describes the geometric analysis techniques employed to visualize latent reasoning trajectories and empirically validate the prevention of latent collapse via SIGReg.

\section{Training}
\label{sec:training_setup}

This section elucidates the configuration and setup used to train the energy-based latent predictive model. The structure of this section is as follows: \cref{sec:model_training_config} shows a detailed configuration setup of the model, with all hyperparameters and additional heuristics used. \cref{sec:dataset_specs} specifies the dataset used to train the model and introduces the different members of the data and how they fit in the framework defined earlier, along with a sample visualization of the data.

\subsection{Model Training Configuration}
\label{sec:model_training_config}
...
\section{Generative Visualization}
\label{sec:generative_viz}
